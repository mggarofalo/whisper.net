...
public static class AppManagementServiceCollectionExtensions
{
	public static IServiceCollection AddAppManagement(this IServiceCollection services, IConfiguration? configuration = null)
	{
		...
		// Delivery-strategy selection: pure override-vs-default precedence for each delivery.
		services.AddSingleton<IDeliveryStrategySelector, DeliveryStrategySelector>();

		// Hotkey activation: the stateful chord matcher that turns key edges into recording
		...
		// Recording state machine: the single authority over Idle/Recording/Transcribing and
		...
		// Capture buffering options: the per-app preroll / max-duration / target-rate the
		// orchestrator builds its CaptureBuffer from. A plain default today; bound from configuration later.
		// The max duration is a SOFT limit (10 minutes by default): recording continues past
		...
		// Dictation orchestrator: the coordination hub that runs capture -> transcribe ->
		...
		// Command-mode hook: the default matcher recognizes nothing, so every transcript falls
		...
		// Hotkey capture + rebinding: the one-shot capture-next-key helper that rebinds the
		...
		// Current settings: the in-memory holder loaded on startup and saved on shutdown.
		// Singleton so every consumer shares one live view of the settings.
		services.AddSingleton<SettingsHolder>();

		// Opt-in audit log gate: reads the live settings holder so enabling/disabling auditing
		...
		// Output transforms: the registry of named transforms (the built-in formats) and the
		...
		// Post-process pipeline: the ordered normalize -> optional transform pipeline behind
		...
		// Dashboard shell + navigation: the navigation service, the shell view-model, and the
		...
		// not transient: the navigation service resolves each section once per shell UI scope
		...
		// First-run setup is no longer a separate flow: the settings shell IS onboarding, opened
		// on launch when the app is unconfigured. There is no dedicated onboarding view-model to register.

		// Self-diagnostics: the doctor / selftest checks, each probing one subsystem through
		...
		// Whisper native runtime: right after the model check — a present model is useless if the
		...
		// User-visible error surfacing: backend failures route a non-technical notice through
	...
	// The host-owned background components. Kept separate from AddAppManagement so the BDD
	...
	public static IServiceCollection AddAppManagementHostedServices(this IServiceCollection services)
	{
		...
		// Hotkey configuration: bind the live matcher to the persisted hotkey at startup and
		...
		// Model warm-up: on launch (and on an active-model switch) preload + warm the
		...
		// Auto-update check: on launch, check the release channel in the background (when the
