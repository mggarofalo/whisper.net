...
shell resolves each section's view-model from the DI container through an
`INavigationService`, so feature views plug in without the shell knowing them.

### View ↔ view-model resolution
...
### Background-thread collection updates
...
- Specs substitute a recording synchronizer, keeping the view-models WPF-free; the relevant
...
### Event wiring: behavior vs command vs legitimate code-behind
...
3. **Legitimate code-behind** stays the narrow exception: a self-contained input control
...
### View-model activation lifecycle

Feature view-models are **cached per shell UI scope**, so navigation toggles activation
...
- **Data sections auto-load on FIRST activation**: a section that shows queried data
  ...
  to the command's `IsRunning`. A section that must re-sync on EVERY activation (Hotkey)
...
The host **owns the application lifetime**. The WPF `App` has no `StartupUri` and shows
...
the relevant scenarios driving a real host over the faked hook seam.

Settings persistence is wired into that same lifecycle: a `SettingsLifecycleService`
...
Persistence is backed by a single local **SQLite** database. Both the `ISettingsStore` and
...
History stays bounded by a retention policy: after each new transcription is recorded, the
...
Each recorded transcription also carries the captured audio duration, persisted alongside
...
**off by default**. The gate is the Logic `AuditLogger`, which reads the live settings holder
...
The tray icon follows the same seam discipline: the coordination — mapping the recording
...
`IHostApplicationLifetime.StopApplication` (the graceful path); Open Settings goes through
...
Single-instance enforcement is the same shape again: the `SingleInstanceCoordinator` in
