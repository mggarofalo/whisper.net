...
// time. It implements exactly what the instant-apply channel's debounce uses — a one-shot timer that is
// disposed and recreated on each staged change — so the debounce can be proven without real wall-clock
// waiting. Not a general-purpose fake (no recurring periods); kept tiny on purpose.
