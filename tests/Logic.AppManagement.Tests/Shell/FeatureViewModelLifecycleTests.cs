...
public sealed class FeatureViewModelLifecycleTests
{
	...
	public FeatureViewModelLifecycleTests() =>
		_viewModel = new HotkeyViewModel(Substitute.For<IMediator>(), _messenger);
	...
	public void Inactive_cached_view_model_gets_no_callbacks()
	{
		...
		_viewModel.CurrentHotkey.Should().BeNull("an inactive cached view-model must not react");
	}
	...
	{
		HotkeyViewModel viewModel = new(Substitute.For<IMediator>(), messenger);
