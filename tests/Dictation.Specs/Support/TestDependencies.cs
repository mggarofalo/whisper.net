...
public static class TestDependencies
{
	...
	public static IServiceCollection CreateServices()
	{
		...
		// load, so scenarios that merely navigate the shell — without configuring the store —
		...
		// load on first activation, so scenarios that merely navigate the shell — without
		...
		// Foreground integrity: default substitute returns Same (the enum's default), so the
		...
		// Delivery routing: the fake factory exposes a typing and a paste injector substitute,
		...
		// Delivery options: a scenario-scoped, mutable instance the strategy driver sets in a
		...
		// Capture: drive the REAL WasapiAudioSource over a fake device seam, so the
		...
		// Global hotkeys: drive the REAL EventLoopHotkeyListener over a fake hook seam, so
		...
		// End-to-end dictation orchestration: the REAL DictationOrchestrator (scoped via
		...
		// Capture tail after chord release: a scenario-scoped manual clock overrides the
		...
		// Long dictation soft limit: a scenario-scoped, mutable holder behind the
		...
		// Command-mode hook: substitute the matcher (default: no match) so the command-mode
		...
		// Continuous dictation mode: the REAL orchestrator over the real audio + delivery
		...
		// Audio feedback: substitute the feedback port so the driver can assert which cue fired
		...
		// Level overlay: the driver builds the real LevelOverlayController (the WPF-free
		...
		// Overlay placement: the driver exercises the real, WPF-free OverlayPlacement geometry
		...
		// UI dispatcher seam: the driver builds the real tray/overlay view-models over their
		...
		// Model picker ports: the catalog + model lifecycle are the real ones from
		...
		// activation, so the ListModels handler must read a contract-respecting status even
		...
		// MVVM shell navigation: the real ShellViewModel + NavigationService + feature
		...
		// Settings/feature view-model foundation: resolve the real feature section view-models
		...
		// Native settings validation: the real HotkeyViewModel over the real Mediator pipeline
		...
		// Instant-apply channel: the messenger + channel are singletons in production, but the
		...
		// Hotkey capture: the WPF-free capture interpreter feeding the validated HotkeyViewModel
		...
		// Audio-device picker: the real AudioDeviceViewModel over the real Mediator pipeline and
		...
		// Model download: the real ModelViewModel over the real Mediator pipeline + catalog with
		...
		// Concurrent model downloads: the real ModelViewModel over the real Mediator pipeline
		...
		// Compact model table: the real ModelViewModel over the real Mediator pipeline +
		...
		// First-run setup decision: the real GetSetupStatusQuery + SwitchActiveModel over the
		...
		// Accessibility: inspects the settings view XAML directly (like the packaging/guidance
		...
		// Native theming: inspects the presentation artifacts to assert the built-in Fluent
		...
		// Sidebar contrast: inspects the presentation artifacts to compute WCAG AA contrast
		...
		// Theme switcher: the real ThemeViewModel over the real Mediator pipeline (GetSettings /
		...
		// View resolution convention: inspects the documented standard, the shell's implicit
		...
		// Declarative event wiring: inspects the views, the reusable focus behavior, the
		...
		// VM activation lifecycle: real shell navigation over the cached feature view-models
		...
		// Section auto-load on first activation: real shell navigation over the cached
		...
		// Error surfacing: the recorder overrides the production IUserNotifier mapping so the
		...
		// View smoke harness: inspects the STA smoke project, the binding-error gate, the
		...
		// Model picker: the real ModelViewModel over the real Mediator pipeline (list /
		...
		// Live history feed: the real HistoryViewModel over the real Mediator pipeline and the
		...
		// History browser: the real HistoryViewModel over the real Mediator pipeline
		...
		// History write-through: re-configures this scenario's IHistoryStore substitute to
		...
		// Stats dashboard: the real StatsViewModel over the real Mediator pipeline
		...
		// Home status dashboard: the real HomeViewModel over the real Mediator pipeline
		...
		// Model warm-up status: the real LevelOverlayController + real HomeViewModel over the
		...
		// flow that also used it was removed; settings is now the single first-run surface).
		services.AddScoped(_ => Substitute.For<IPermissionProbe>());

		// Host bootstrapping: the driver builds its own real Generic Host internally over a
		...
		// Settings persistence: the driver composes the real lifecycle service over the real
		...
		// SQLite persistence store: the driver exercises the real migration runner + SQLite
		...
		// History retention + paged browsing: the driver builds its own real Mediator pipeline
		...
		// Usage stats local-day bucketing: the driver exercises the real UsageStatsCalculator
		...
		// Usage stats recording + aggregation: the driver builds its own real Mediator pipeline
		...
		// Opt-in audit log: the driver builds its own composition — the real AuditLogger gate +
		...
		// Run on login: drive the real command/query handlers through IMediator over an
		...
		// Single-instance enforcement: drive the real coordinator over fake lock + signal
		...
		// Hotkey activation modes: the real HotkeyActivationController behind the driver.
		...
		// Recording state machine: the real machine behind the driver, scoped so each
		...
		// Hotkey rebinding: the real capture service + controller behind the driver, scoped
		...
		// Text delivery: the real SendInputTextInjector over a recording fake keyboard seam.
		services.AddScoped<TextInjectionDriver>();

		// Clipboard fallback: the real ClipboardTextInjector over fake clipboard + keyboard seams.
		services.AddScoped<ClipboardDeliveryDriver>();

		// UIPI-aware delivery: the real pipeline through IMediator with the integrity probe faked.
		services.AddScoped<UipiDeliveryDriver>();

		// Delivery-strategy selection: the real pipeline + selector, routing to the fake factory.
		...
		// Transcription normalization: the real IFillerWordCleaner behind the driver.
		...
		// Opt-in localhost rephrase: the real OllamaRephraseClient + validator over a
		...
		// Output transforms: the real OutputTransformService + registry over a faked rephrase
		...
		// Post-process pipeline: the real pipeline behind IPostProcessor, overridden to scoped
		...
		// On-device transcription: the real Whisper.net adapter over a fake engine seam.
		services.AddScoped<WhisperTranscriptionDriver>();

		// Model registry/cache/download: real catalog + cache + downloader, hermetic source.
		services.AddScoped<ModelLibraryDriver>();

		// Model lifecycle: the real lifecycle policy over a fake runtime.
		services.AddScoped<ModelLifecycleDriver>();

		// Device selection: fake enumerator + notification client behind the driver.
		...
		// Audio device + hotkey configuration: bind the device-enumerator port to the fake so
		...
		// Self-diagnostics: the real doctor pipeline — RunDiagnosticsQuery through the
		...
		// Velopack packaging configuration: inspects repository artifacts (the project file,
		...
		// Tag-driven release workflow: inspects .github/workflows/release.yml directly.
		services.AddScoped<ReleaseWorkflowDriver>();

		// Self-signed code signing for personal builds: inspects the signing-cert helper
		...
		// Diagnosable logging: drives the real AddSerilogLogging composition pointed at a temp
		// directory and asserts the event lands in a rolling log file on disk.
		services.AddScoped<LoggingDriver>();

		// App-data location: drives the real AddInfrastructure composition to resolve the model
		...
		// Hotkey reassignment: drives the real HotkeyConfigurationHostedService + activation
		...
		// Hotkey assignment end-to-end: the real HotkeyViewModel entered through the REAL
		...
		// Signed auto-update: the real AutoUpdateService policy over a faked update source, so
